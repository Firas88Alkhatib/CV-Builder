  ...
  return (
    <View style={{ ...styles.personalDetails, backgroundColor: color }}>
      ...
      <Text style={styles.details}>
        {Object.entries((({ firstName, lastName, jobTitle, ...rest }) => rest)(personalDetails)).filter(([k, v]) => !!v).length !== 0 && 'Details'}
      </Text>
      <Text>{personalDetails.address}</Text>
      <Text>{`${personalDetails.city}${personalDetails.city && personalDetails.postalCode && ', '}${personalDetails.postalCode}`}</Text>
